...
class $modify(CommentCell) {
    void loadFromComment(GJComment * comment) {
        ...
        TextArea* textArea = typeinfo_cast<TextArea*>(mainLayer->getChildByID("comment-text-area"));
        ...
        CCARRAY_FOREACH(mainLayer->getChildren(), obj) {
            CCSprite* spr = typeinfo_cast<CCSprite*>(obj);
            if (spr && !typeinfo_cast<SimplePlayer*>(obj)) {
        ...
        if (isProfileComment)
            ...
            menuItem = usernameClickable ? typeinfo_cast<CCMenuItemSpriteExtra*>(mainMenu->getChildByID("username-button"))
            ...
            if (usernameClickable) {
                ...
                auto deleteButton = typeinfo_cast<CCMenuItemSpriteExtra*>(mainMenu->getChildByID("delete-button"));
        ...
        CCARRAY_FOREACH(commentLabels, obj) {
            CCLabelBMFont* lbl = typeinfo_cast<CCLabelBMFont*>(obj);
            if (!lbl) continue;
